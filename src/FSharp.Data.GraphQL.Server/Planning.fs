    ...
    Define.Field(
        ...
        args = [
            { Name = "name"
              ...
              TypeDef = String
              DefaultValue = None
              ExecuteInput = variableOrElse(fun metadata value -> coerceStringInput metadata value |> Option.map box) }
        ],
...
let private directiveIncluder (directive: Directive) : Includer =
    let coerceDirectiveInput (value : Value) (metadata : Metadata) : bool =
        match coerceBoolInput metadata value with
        | Some s -> s
        | None -> raise (GraphQLException (sprintf "Expected 'if' argument of directive '@%s' to have boolean value but got %A" directive.Name value))
    fun variables metadata ->
        match directive.If.Value with
        | Variable vname ->
            match Map.tryFind vname variables with
            | Some value -> coerceDirectiveInput value metadata
            | None -> failwithf "Expected 'if' argument of directive '@%s' to have a non-nullable boolean value, but got null" directive.Name
        | other -> coerceDirectiveInput other metadata

let private incl: Includer = fun _ _ -> true
let private excl: Includer = fun _ _ -> false
let private getIncluder (directives: Directive list) parentIncluder : Includer =
    ...
    |> List.fold (fun acc directive ->
        ...
        | "skip" ->
            fun vars metadata -> acc vars metadata && not(directiveIncluder directive vars metadata)
        | "include" ->
            fun vars metadata -> acc vars metadata && (directiveIncluder directive vars metadata)
