...
let inline private argVal argDef argOpt vars metadata =
    match argOpt with
    | Some arg -> Execution.argumentValue argDef arg vars metadata
...
let private resolveLinqArg vars metadata (name, argdef, arg) =
    argVal argdef arg vars metadata |> Option.map (fun v -> { Arg.Name = name; Value = v })
...
let private linqArgs vars metadata info =
    ...
    if Array.isEmpty argDefs then []
        ...
        |> Array.choose (resolveLinqArg vars metadata)
        |> Array.toList
...
/// </para>
/// </summary>
let rec tracker (vars: Map<string, Value>) (metadata : Metadata) (info: ExecutionInfo) : Tracker  =
    let ir = getTracks Set.empty info
    let composed = compose vars metadata ir
    let (IR(_, trackers, children)) = composed
    join trackers children |> Seq.head

let private toLinq info (query: IQueryable<'Source>) (variables : Map<string, Value>) (metadata : Metadata) (argApplicators: Map<string, ArgApplication>) : IQueryable<'Source> =
    let parameter = Expression.Parameter (query.GetType())
    let ir = tracker variables metadata info
...
type System.Linq.IQueryable<'Source> with
    ...
    member this.Apply(info: ExecutionInfo, ?variables: Map<string, Value>, ?metadata : Metadata, ?applicators: Map<string, ArgApplication>) : IQueryable<'Source> =
        let appl =
        ...
        toLinq info this (defaultArg variables Map.empty) (defaultArg metadata (Metadata(Map.empty))) appl
